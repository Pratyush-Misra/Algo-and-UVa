...
void solve(){
	...
	FOR(x,L[0],10005){
		int mx = 0;
		FOR(curr, N){
			if(x >= L[curr] and x < R[curr])	umax(mx, H[curr]);	
			umax(last_x,R[curr]);
		}
		ht_map[x] = mx;
	}
	//cout << last_x << endl;
	//FOR(last_x+1)	cout << i << "-> " << ht_map[i] << endl;
	int ht = -1;
	for(int x = 1; x <= last_x + 1; x++)
		if(ht != ht_map[x]){
			cout << x << ' ' << ht_map[x] << ' ';
			ht = ht_map[x];
		}
...
int main() {
	...
	FOR(1){
		cin >> l >> h >> r;
		L.pb(l);	H.pb(h);	R.pb(r);
	}
	solve();
	// int l = rand() % 1000 + 1;
	// FOR(30){
	// 	cout << ++l + 5 << ' ' << rand() % 1000 + 10 << ' ' << rand() % 1000 + l + 3 << endl;
	// }
